...
function age_report() {
	...
	foreach($fourcodes as $fourcode) {
		...
		$lnk = "<a href='http://198.74.49.22/get_fed_page_t.php?id=$fourcode' target='_blank'>$fourcode</a>";		
		$tablebody .= "
				<tr class='rowsearch'>
					<td>$lnk</td>
					<td>$incumbent</td>
...
function getcds() {
	...
	global $ctb2016_conn;
	$conn = $ctb2016_conn;
	$sql = "SELECT FOURCODE FROM dp02 ORDER BY FOURCODE ASC";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$thiscode = $row['FOURCODE'];
			array_push($retval, $thiscode);
		}
	}
	return $retval;	
}
...
function get_census_ages($fourcode) {
	global $ctb2016_conn;
	$conn = $ctb2016_conn;
	$sql = "SELECT * FROM dp05 WHERE FOURCODE = '$fourcode'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
...
function get_census_school($fourcode) {
	global $ctb2016_conn;
	$conn = $ctb2016_conn;
	$sql = "SELECT * FROM dp02 WHERE FOURCODE = '$fourcode'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
...
function get_census_occupation($fourcode) {
	global $ctb2016_conn;
	$conn = $ctb2016_conn;
	$sql = "SELECT * FROM dp03 WHERE FOURCODE = '$fourcode'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
...
function get_census_income($fourcode) {
	...
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			$tmp['VC118'] = $row['HC01_VC118']; //PER CAPITA INCOME
			$tmp['VC171'] = $row['HC03_VC171']; //BELOW POVERTY LEVEL
...
function get_census_housing($fourcode) {
	global $ctb2016_conn;
	$conn = $ctb2016_conn;
	$sql = "SELECT * FROM dp04 WHERE FOURCODE = '$fourcode'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
...
function getofficeholder($fourcode){
	global $ctb2016_conn;
	$conn = $ctb2016_conn;
	$sql = "SELECT NAML, NAMF, PARTY FROM e18_fed WHERE DIST = '$fourcode'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
