...
@section('title', "Federal Campaign Finance - 2018 - $state")
    ...
    <div class='container'>
        <div class='row'>
            <div class='col-lg-12'>
                ...
                $sql = "SELECT CAND_ID, CAND_NAME, CAND_PTY_AFFILIATION, CAND_ELECTION_YR, CAND_OFFICE_ST, CAND_OFFICE, CAND_OFFICE_DISTRICT, CAND_PCC, CAND_ICI FROM nufec18_cn WHERE (CAND_ELECTION_YR = '2018' || CAND_ELECTION_YR = '2017') && CAND_OFFICE_ST = '$thisstate' ORDER BY CAND_OFFICE_ST, CAND_OFFICE, CAND_OFFICE_DISTRICT ASC";
...
//                echo($tablehead);
		...
		";
                                    ...
                                    if ($distietotal) {
                                        ...
                                        if ($ie_cmte[$fourcode]['ps'] || $ie_cmte[$fourcode]['po']) {
                                            ...
                                            foreach ($ie_cmte[$fourcode]['ps'] as $cmte) {
                                                ...
                                                $tmp_arr[$id] = "<p class='boldme'>$committee_link spent \$" . number_format($cmte['amount']) . " <span class='boldme greenme'> in Support of </span> " . $cmte['cand'];
                                            }

                                            foreach ($ie_cmte[$fourcode]['po'] as $cmte) {
                                                $id = $cmte['cmte_id'];
                                                $committee_link = "<a href='http://198.74.49.22/fediecmtedetail.php?id==" . $cmte['cmte_id'] . "' target='_blank'>" . $cmte['cmte_nm'] . "</a>";
                                                $tmp_arr[$id] .= "<br><span class='boldme'>$committee_link spent \$" . number_format($cmte['amount']) . " <span class='boldme redme'> in Opposition to </span> " . $cmte['cand'] . "</span></p>";
                                            }

                                            foreach($tmp_arr as $x) {
                                                $distie .= $x;
                                            }
                        ...
                        if ($thiscmte) {
                            $getfeccandidateinfo = getfeccand($thiscmte);
    		...
    		";
                ...
                <table id='$thisid' style='margin-top: 20px' class='bordered tablesorter table-hover tablesaw tablesaw-stack' data-tablesaw-mode='stack'>
                    <thead>
                        <tr>
                            <th>ELECTION</th>
                            <th>COMMITTEE</th>
            ...
            ";
                            ...
                            $distie .= "<div class='newseg'><h2 align='center'>Independent Expenditures Affecting $thiscand_name: \$" . number_format($totalcand, 2) . "</h2>
...
";
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            if ($election == "S" || $election == "O") {
                                $election = "G";
                            }

                            if ($election == "G") {
                                $gentotal += $amount;
                                if ($support == "S" || $support == "Support") {
                                    $gensupport += $amount;
                                ...
                                } elseif ($support == "O" || $support == "Oppose") {
                                    $genoppose += $amount;
