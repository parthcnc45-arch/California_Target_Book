...
@section('title', 'CA IE Filings (2017 Off-Year) | California Target Book')
...
@section('content')
    ...
    $sql = "SELECT * FROM calaccess_raw_CVR_CAMPAIGN_DISCLOSURE_CD WHERE FILING_ID > '2116221' && (DIST_NO <> '' || BAL_NUM <> '') && FORM_TYPE = 'F496' ORDER BY FILING_ID DESC, AMEND_ID DESC";
    $result = $conn->query($sql);

    echo("<div class='newseg'>");
    echo("<h1>2017 Independent Expenditure Filings</h1>");
...
";
	...
	";
		...
		<div class='newseg' style='margin-top: 20px;'>
			...
			<h3>Top Donors Since January 1st, 2017</h3>
	");
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
    ...
    {
        global $conn;
        $sql = "SELECT FOURCODE FROM calaccess_raw_e18_comm WHERE NAML LIKE '%$name%' && FOURCODE LIKE '%$district%'";
        //echo("<br>$sql<br");
        $result = $conn->query($sql);
        $retval = False;
        if ($result->num_rows > 0) {
        ...
        if (mb_substr($retval, 0, 2) == "CD" || mb_substr($retval, 0, 2) == "CA") {
            $retval = '';
        }
