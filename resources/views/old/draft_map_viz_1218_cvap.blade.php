...
<div class='container-fluid pt-lg' height='100%'>
    <h2>
        December 18th CD Visualization Citizen Voting Age Population (CVAP) Data
    </h2>
...
function get_cvap($dists) {
	...
	$jur_name = "VIZ7_";
	$sql = "SELECT district, name, jur_name FROM ctb_ca_city_shp WHERE jur_name = 'VIZ7_CD'";
	//echo("\n$sql");
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
	...
	$sql = "SELECT name, population, cvap_19, hisp, ind, black, asian, white, target, deviation FROM ctb_redist_cvap_1218";
	//echo("\n$sql");	
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
