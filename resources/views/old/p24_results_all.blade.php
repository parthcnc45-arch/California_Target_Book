...
@section('title', 'March 3rd, 2024 Primary Results - All | California Target Book')
...
<div class='container-fluid pt-lg' height='100%'>
    <h2>
        March 3rd, 2024 Primary Results - All
    </h2>
...
$enddraw_nav = '
	<nav class="clearfix page-nav">
		...
		<li><a href="#PROP" role="tab" data-toggle="tab">Props  </a></li>
		...
		<li><a href="#POTUS-D" role="tab" data-toggle="tab" >Presidential Primary (D) by CD</a></li>
		<li><a href="#POTUS-R" role="tab" data-toggle="tab" >Presidential Primary (R) by CD</a></li>
...
foreach($end_arr as $fourcode => $ranks) {
	if(mb_substr($fourcode, 0, 1) == ".") {
		...
		$type = "BOE";
	} elseif ($fourcode == "POTUS-D_ALL" || $fourcode == "POTUS-R_ALL") {
		$type = "STW";
	} elseif(mb_substr($fourcode, 0, 2) == "PR") {
		$type = "PROP";
	} elseif(mb_substr($fourcode, 0, 7) == "POTUS-D") {
		$type = "POTUS-D";
	} elseif(mb_substr($fourcode, 0, 7) == "POTUS-R") {
		$type="POTUS-R";
	} else {
		$type = mb_substr($fourcode, 0, 2);
	}
	$total = $end_arr_other[$fourcode]['total'];
	$r_vot = $end_arr_other[$fourcode]['party_votes']['Rep'];
	$d_vot = $end_arr_other[$fourcode]['party_votes']['Dem'];
	$d_pct = number_format((($end_arr_other[$fourcode]['party_votes']['Dem'] / $total) * 100), 2);
	$r_pct = number_format((($end_arr_other[$fourcode]['party_votes']['Rep'] / $total) * 100), 2);
	...
	<table>
		<tbody>";
	foreach($ranks as $rank => $x) {
		array_push($all_arr, $tmp);
		$enddraw[$type] .= "<tr>
						<td>" . $x['cand_nm'] . "</td>
						<td>" . $x['party'] . "</td>
...
$enddraw['ALL'] .= "</tbody></table>";
echo($enddraw_nav);

$types = Array("STW", "PROP", "AD", "SD", "CD", "POTUS-D", "POTUS-R", "ALL");
$is_active = TRUE;
...
function populate_last() {
	...
	$sql = "SELECT * FROM (
					SELECT fourcode, update_number FROM ctb_p24_results_state
	...
	$sql = "SELECT * FROM ctb_p24_results_state 
			WHERE ( $query )";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
