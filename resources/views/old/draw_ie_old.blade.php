...
if(isset($id)) {
  ...
  global $fourcode, $cycle, $filing_arr, $filer_arr, $filer_name_arr, $short_cycle, $cycle_yr1, $cycle_yr2, $cand_ccl;
  if(is_int($id)) {
	$year = $id;
	$fourcode = $year;
  } else {
...
$key_name = "IE_" . $fourcode . "_v2_" . $year;
$key_name_js = "IE_" . $fourcode . "_js_" . $year;
$key_name_hd = "IE__" . $fourcode . "_hd_" . $year;

if($year != "2020" && $year != "2022") {

    if(Cache::has($key_name)) {

    	$head = Cache::get($key_name_hd);
    	echo($head);

        $final_draw = Cache::get($key_name);
        echo($final_draw);

        $js = Cache::get($key_name_js);
        echo($js);
...
foreach($candidates as $c) {
	...
	if(isset($cand_table[$party_class]['table_body'])) {
	...
	if(isset($cand_party_collision[$cand_naml])) {
		$cand_party_collision[$cand_naml]++;
	} else {
		$cand_party_collision[$cand_naml] = 1;
	}
...
foreach($cand_finances as $cand_id => $c) {
	...
	$table_body['cand_finances'] .= "<tr>
										...
										<td>" . $cand_span      . "</td>
										<td>" . (isset($c['filed']) ? $c['filed'] : "")   . "</td>
										<td>" . ($c['cmte_nm'] ?? '') . "</td>
										<td>" . $cmte_span . "</td>
										<td align='right'>" . (isset($c['rcpt']) ? number_format($c['rcpt']) : "")    . "</td>
										<td align='right'>" . (isset($c['loan']) ? number_format($c['loan']) : "")    . "</td>
										<td align='right'>" . (isset($c['expn']) ? number_format($c['expn']) : "")    . "</td>
										<td align='right'>" . (isset($c['coh']) ? number_format($c['coh']) : "")      . "</td>
										<td align='right'>" . (isset($c['debt']) ? number_format($c['debt']) : "")    . "</td>
										<td>" . (isset($c['pd_end']) ? $c['pd_end'] : "")  . "</td>
									</tr>";
}
...
if(isset($transactions)) {
	...
	foreach($transactions as $t) {
		...
		$cand_party = '';
		if(!empty($cand_party_arr[$cand_naml])) {
		...
		if($amt < 100) {
			...
			if(!empty($cand_party_collision[$cand_naml])) {
			...
			$table_body['ie_detail'] .= "<tr>
											<td>" . (isset($t['fourcode'])?$t['fourcode']:"") . "</td>
											<td align='right'>$filing_span</td>
											<td>$filer_span</td>
											<td align='right'>" . $t['exp_date'] . "</td>
											<td align='right'>" . number_format($t['amount']) . "</td>
											<td>" . (isset($t['vendor'])?mb_substr($t['vendor'], 0, 64):"") . "</td>
											<td>" . (isset($t['dscr'])?mb_substr($t['dscr'], 0, 64):"") . "</td>
											<td>" . (isset($t['transaction_id'])?$t['transaction_id']:"") . "</td>
											<td>" . (isset($t['election_type'])?$t['election_type']:"") . "</td>
											<td class='$position'>" . (isset($t['position'])?$t['position']:"") . "</td>
											<td class='$cand_party'>" . (isset($t['cand_nm'])?$t['cand_nm']:"") . "</td>
										</tr>";
		}

$filer_index[$filer_id]['filer_nm']										= $filer_nm;
		(isset($filer_totals[$filer_id]['ALL']['TOTAL']) 								? $filer_totals[$filer_id]['ALL']['TOTAL'] += $t['amount'] 						: $filer_totals[$filer_id]['ALL']['TOTAL'] = $t['amount']);
		(isset($filer_totals[$filer_id][$election_class]['TOTAL'])						? $filer_totals[$filer_id][$election_class]['TOTAL'] += $t['amount'] 		    : $filer_totals[$filer_id][$election_class]['TOTAL'] = $t['amount']);

		(isset($filer_election_totals[$election_class][$filer_id]['TOTAL']) 			? $filer_election_totals[$election_class][$filer_id]['TOTAL'] += $t['amount'] 	: $filer_election_totals[$election_class][$filer_id]['TOTAL'] = $t['amount']);
		(isset($filer_election_totals['ALL'][$filer_id]['TOTAL'])						? $filer_election_totals['ALL'][$filer_id]['TOTAL'] += $t['amount']				: $filer_election_totals['ALL'][$filer_id]['TOTAL'] = $t['amount']);

		(isset($totals['fourcode'][$fourcode]) 											? $totals['fourcode'][$fourcode] += $t['amount']								: $totals['fourcode'][$fourcode] = $t['amount']);
		...
		(isset($totals['filers'][$filer_id]['summary']['TOTAL'])						? $totals['filers'][$filer_id]['summary']['TOTAL'] += $t['amount'] : $totals['filers'][$filer_id]['summary']['TOTAL'] = $t['amount']);
		(isset($totals['filers'][$filer_id]['elections'][$election_class])				? $totals['filers'][$filer_id]['elections'][$election_class] += $t['amount'] : $totals['filers'][$filer_id]['elections'][$election_class] = $t['amount']);
		(isset($totals['filers'][$filer_id]['candidates'][$cand_naml]['TOTAL'])			? $totals['filers'][$filer_id]['candidates'][$cand_naml]['TOTAL'] += $t['amount'] : $totals['filers'][$filer_id]['candidates'][$cand_naml]['TOTAL'] = $t['amount']);
		(isset($totals['filers'][$filer_id]['candidates'][$cand_naml][$election_class])	? $totals['filers'][$filer_id]['candidates'][$cand_naml][$election_class] += $t['amount'] : $totals['filers'][$filer_id]['candidates'][$cand_naml][$election_class] = $t['amount']);	
		(isset($totals['filers'][$filer_id]['position'][$position])						? $totals['filers'][$filer_id]['position'][$position] += $t['amount'] : $totals['filers'][$filer_id]['position'][$position] = $t['amount']);
		(isset($totals['filers'][$filer_id]['candidates_spent_on'][$position][$cand_naml])	? $totals['filers'][$filer_id]['candidates_spent_on'][$position][$cand_naml]+= $t['amount'] : $totals['filers'][$filer_id]['candidates_spent_on'][$position][$cand_naml] = $t['amount']);		
		(isset($totals['filers'][$filer_id]['election_candidates'][$election_class][$cand_full]['TOTAL']) ? $totals['filers'][$filer_id]['election_candidates'][$election_class][$cand_full]['TOTAL'] += $t['amount'] : $totals['filers'][$filer_id]['election_candidates'][$election_class][$cand_full]['TOTAL'] = $t['amount']);
		(isset($totals['filers'][$filer_id]['election_candidates_naml'][$election_class][$cand_full]['TOTAL']) ? $totals['filers'][$filer_id]['election_candidates_naml'][$election_class][$cand_full]['TOTAL']+= $t['amount'] : $totals['filers'][$filer_id]['election_candidates_naml'][$election_class][$cand_full]['TOTAL'] = $t['amount']);
		


		

		(isset($totals['dates'][$exp_dat])													? $totals['dates'][$exp_dat]+= $t['amount']:$totals['dates'][$exp_dat]= $t['amount']);
		(isset($totals['election_dates'][$election_class][$exp_dat])						? $totals['election_dates'][$election_class][$exp_dat] += $t['amount']							:$totals['election_dates'][$election_class][$exp_dat] = $t['amount']);

		(isset($totals['grand_total'])														? $totals['grand_total']+= $t['amount']															:$totals['grand_total']= $t['amount']);

		(isset($totals['candidates'][$cand_naml]['summary']['TOTAL'])						? $totals['candidates'][$cand_naml]['summary']['TOTAL'] += $t['amount']:$totals['candidates'][$cand_naml]['summary']['TOTAL'] = $t['amount']);
		(isset($totals['candidates'][$cand_naml]['summary'][$election_class]['TOTAL'])		? $totals['candidates'][$cand_naml]['summary'][$election_class]['TOTAL']+= $t['amount']:$totals['candidates'][$cand_naml]['summary'][$election_class]['TOTAL']= $t['amount']);
		(isset($totals['candidates'][$cand_naml]['summary'][$election_class][$position])	? $totals['candidates'][$cand_naml]['summary'][$election_class][$position]+= $t['amount']:$totals['candidates'][$cand_naml]['summary'][$election_class][$position]= $t['amount']);
		(isset($totals['candidates'][$cand_naml]['position'][$position]['TOTAL'])			? $totals['candidates'][$cand_naml]['position'][$position]['TOTAL']+= $t['amount']:$totals['candidates'][$cand_naml]['position'][$position]['TOTAL']= $t['amount']);
		(isset($totals['candidates'][$cand_naml]['position'][$position][$election_class])	? $totals['candidates'][$cand_naml]['position'][$position][$election_class]+= $t['amount']:$totals['candidates'][$cand_naml]['position'][$position][$election_class]= $t['amount']);

		(isset($totals['candidates_election'][$election_class][$cand_naml][$position])		? $totals['candidates_election'][$election_class][$cand_naml][$position]+= $t['amount']:$totals['candidates_election'][$election_class][$cand_naml][$position]= $t['amount']);
		(isset($totals['candidates_election']['ALL'][$cand_naml][$position])				? $totals['candidates_election']['ALL'][$cand_naml][$position]+= $t['amount']:$totals['candidates_election']['ALL'][$cand_naml][$position]= $t['amount']);

		(isset($totals['candidates_full'][$cand_nm]['summary']['TOTAL'])					? $totals['candidates_full'][$cand_nm]['summary']['TOTAL']+= $t['amount']:$totals['candidates_full'][$cand_nm]['summary']['TOTAL']= $t['amount']);
		(isset($totals['candidates_full'][$cand_nm]['summary'][$election_class])			? $totals['candidates_full'][$cand_nm]['summary']['TOTAL']+= $t['amount']:$totals['candidates_full'][$cand_nm]['summary']['TOTAL']= $t['amount']);
		(isset($totals['candidates_full'][$cand_nm]['position'][$position]['TOTAL'])		? $totals['candidates_full'][$cand_nm]['position'][$position]['TOTAL']+= $t['amount']:$totals['candidates_full'][$cand_nm]['position'][$position]['TOTAL']= $t['amount']);
		(isset($totals['candidates_full'][$cand_nm]['position'][$position][$election_class])? $totals['candidates_full'][$cand_nm]['position'][$position][$election_class]+= $t['amount']:$totals['candidates_full'][$cand_nm]['position'][$position][$election_class]= $t['amount']);

		(isset($totals['position_total'][$position])										? $totals['position_total'][$position]+= $t['amount']:$totals['position_total'][$position]= $t['amount']);

		(isset($totals['position_cands'][$position][$cand_naml]['TOTAL'])					? $totals['position_cands'][$position][$cand_naml]['TOTAL']+= $t['amount']:$totals['position_cands'][$position][$cand_naml]['TOTAL']= $t['amount']);
		(isset($totals['position_cands'][$position][$cand_naml][$election_class])			? $totals['position_cands'][$position][$cand_naml][$election_class]+= $t['amount']:$totals['position_cands'][$position][$cand_naml][$election_class]= $t['amount']);

		(isset($totals['elections'][$election_class]['summary']['TOTAL'])					? $totals['elections'][$election_class]['summary']['TOTAL']+= $t['amount']:$totals['elections'][$election_class]['summary']['TOTAL']= $t['amount']);
		(isset($totals['elections'][$election_class]['summary'][$position])					? $totals['elections'][$election_class]['summary'][$position]+= $t['amount']:$totals['elections'][$election_class]['summary'][$position]= $t['amount']);
		(isset($totals['elections'][$election_class]['filers'][$filer_id])					? $totals['elections'][$election_class]['filers'][$filer_id]+= $t['amount']:$totals['elections'][$election_class]['filers'][$filer_id]= $t['amount']);

		(isset($totals['by_date'][$election_class][$exp_dat])								? $totals['by_date'][$election_class][$exp_dat]+= $t['amount']:$totals['by_date'][$election_class][$exp_dat]= $t['amount']);
		(isset($totals['by_date']['ALL'][$exp_dat])											? $totals['by_date']['ALL'][$exp_dat]+= $t['amount']:$totals['by_date']['ALL'][$exp_dat]= $t['amount']);

		(isset($totals['by_name'][$election_class][$cand_naml][$position])					? $totals['by_name'][$election_class][$cand_naml][$position]+= $t['amount']:$totals['by_name'][$election_class][$cand_naml][$position]= $t['amount']);
		(isset($totals['by_name']['ALL'][$cand_naml][$position])							? $totals['by_name']['ALL'][$cand_naml][$position]+= $t['amount']:$totals['by_name']['ALL'][$cand_naml][$position]= $t['amount']);
...
$page['ie_cmte_totals'] .= "<h1>TOTAL INDEPENDENT EXPENDITURES: \$" . (isset($totals['grand_total']) ? number_format($totals['grand_total']) : "") . "</h1>";

if(isset($totals['elections']['Primary']['summary']['TOTAL'])) {
	$e_breakout .= "\$" . number_format($totals['elections']['Primary']['summary']['TOTAL']) . " in Primary Election<br>";
}
if(isset($totals['elections']['General']['summary']['TOTAL'])) {
	$e_breakout .= "\$" . number_format($totals['elections']['General']['summary']['TOTAL']) . " in General Election<br>";	
}

if(isset($totals['elections']['Special/Other']['summary']['TOTAL'])) {
...
array_push($endjava, $js);
$table_body['ie_cmte_detail'] = '';
$table_head['ie_cmte_detail'] = '';
$table_end['ie_cmte_detail'] = '';
if(isset($totals['grand_total'])) {
	...
	foreach($filer_totals as $filer_id => $v) {
		$support_cands = '';
		$oppose_cands = '';
		if(isset($totals['filers'][$filer_id]['candidates_spent_on']['Support'])) {
			foreach($totals['filers'][$filer_id]['candidates_spent_on']['Support'] as $cand_nm => $amt) {
				if(!empty($cand_party_arr[$cand_nm])) {
		...
		if(isset($totals['filers'][$filer_id]['candidates_spent_on']['Oppose'])) {
		...
		$table_body['ie_cmte_detail'] .= "<tr>
											<td>" . $filer_span . "</td>
											<td>" . $filer_id . "</td>
											<td align='right'>" . (isset($totals['filers'][$filer_id]['summary']['TOTAL'])?number_format($totals['filers'][$filer_id]['summary']['TOTAL']):"") . "</td>
											<td align='right'>" . (isset($filer_totals[$filer_id]['Primary']['TOTAL'])?number_format($filer_totals[$filer_id]['Primary']['TOTAL']):"") . "</td>
											<td align='right'>" . (isset($filer_totals[$filer_id]['General']['TOTAL'])?number_format($filer_totals[$filer_id]['General']['TOTAL']):"") . "</td>
											<td align='right'>" . (isset($filer_totals[$filer_id]['Special/Other']['TOTAL'])?number_format($filer_totals[$filer_id]['Special/Other']['TOTAL']):"") . "</td>
											<td>$support_cands</td>
											<td>$oppose_cands</td>
										</tr>";
	}
...
$page['ie_cmte_totals'] .= "<div class='ie_list_div'>
							...
							(isset($totals['grand_total']) ? number_format($totals['grand_total']) : "") . "</b></h3></p>
...
if (isset($filer_election_totals)) {
    if (is_array($filer_election_totals)) {
        uasort($filer_election_totals, "election_sort");
    } else {
        echo "Error: \$filer_election_totals is not an array\n";
    }
}
...
foreach ($etypes as $etype) {
    if (isset($totals['elections'][$etype]['summary']['TOTAL'])) {
        if (!is_array($totals['elections'])) {
            echo "Error: \$totals['elections'] is not an array\n";
            continue;
        }
        if (!is_array($totals['elections'][$etype])) {
            echo "Error: \$totals['elections'][$etype] is not an array\n";
            continue;
        }
        if (!is_array($totals['elections'][$etype]['summary'])) {
            echo "Error: \$totals['elections'][$etype]['summary'] is not an array\n";
            continue;
        }
        ...
        if (isset($filer_election_totals[$etype]) && is_array($filer_election_totals[$etype])) {
            ...
            foreach ($filer_election_totals[$etype] as $filer_id => $amt) {
                ...
                if (isset($totals['filers'][$filer_id]['election_candidates'][$etype]) && is_array($totals['filers'][$filer_id]['election_candidates'][$etype])) {
                    ...
                    echo "Error: \$totals['filers'][$filer_id]['election_candidates'][$etype] is not an array\n";
                }
            }
        } else {
            echo "Error: \$filer_election_totals[$etype] is not an array or does not exist\n";
        }

        $page['ie_election_totals'] .= "</p>
            </div>";

        if (empty($totals['candidates_election'][$etype])) {
            $totals['candidates_election'][$etype] = [];
        }

	
        if (is_array($totals['candidates_election'][$etype])) {
	    /*
            foreach ($totals['candidates_election'][$etype] as $cand_naml => $x) {
                if (empty($chart_data[$supopp_function])) {
...
foreach($etypes as $etype) {
	...
	if(isset($totals['election_dates'][$etype])) {
...
$ie_detail_page .= "<div id='ie_detail_table_page_$cycle' align='center' class='tab-pane'><h1>INDEPENDENT EXPENDITURES FOR $master_fourcode ($cycle Cycle)<br>\$" . (isset($totals['grand_total']) ? number_format($totals['grand_total']) : "") . "</h1>$e_breakout" . $page['ie_detail'] . "</div>";
$ie_detail_page .= "</div>";
$ie_detail_page .= "</div>";
...
function get_state_transactions($candidates, $fourcode) {
	...
	if($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			$filer_arr[$filer_id][$this_filing]['cand_namf'] = $x['cand_namf'];
			$filer_arr[$filer_id][$this_filing]['cand_naml'] = $x['cand_naml'];
...
function get_candidates($fourcode, $cycle, $usestate, $usefed) {
	...
	if($usestate) {
		if($cycle > 2018 && $cycle < 2021) {
			...
			$sql = "SELECT * FROM ctb_cand_filed_v2 WHERE fourcode = '$fourcode' && cycle = '$cycle' && hide != 1 GROUP BY cand_id";
			$result = $conn->query($sql);
			if($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					$tmp['cand_naml'] = $row['naml'];
			...
			$sql = "SELECT * FROM calaccess_raw_e22_comm WHERE fourcode = '$fourcode' && hide != 1 GROUP BY cand_id";
			$result = $conn->query($sql);
			if($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					$tmp['cand_naml'] = $row['naml'];
...
function get_state_committees($candidates, $fourcode) {
	...
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			if(!isset($filer_last[$cmte_id]['pd_end'])) {
	...
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			switch($row['LINE_ITEM']) {
				case "5":
					$rcpt = $row['AMOUNT_A'];
					(isset($retval[$cand_id]['rcpt']) ? $retval[$cand_id]['rcpt'] += $rcpt :  $retval[$cand_id]['rcpt'] = $rcpt);
					$retval[$cand_id]['rcpt'] += $rcpt;
					break;
				case "11":
					$expn = $row['AMOUNT_A'];
					(isset($retval[$cand_id]['expn']) ? $retval[$cand_id]['expn'] += $expn :  $retval[$cand_id]['expn'] = $expn);
					break;
				case "16":
...
if($year < 2019) {
        Cache::forever($key_name_js, $output);
}
...
$head = "<html lang='en'>
    ...
    <head>
        ...
        <title>IE Single | California Target Book</title>
...
if($year < 2019) {
        Cache::forever($key_name_hd, $output);
}
