    ...
    <div class='container'>
        <div class='row'>
            <div class='col-lg-12'>
                ...
                $sql = "SELECT CAND_ID, CAND_NAME, CAND_PTY_AFFILIATION, CAND_ELECTION_YR, CAND_OFFICE_ST, CAND_OFFICE, CAND_OFFICE_DISTRICT, CAND_PCC, CAND_ICI FROM nufec_cn WHERE CAND_ELECTION_YR = '2016' && CAND_OFFICE_ST = '$thisstate' ORDER BY CAND_OFFICE_ST, CAND_OFFICE, CAND_OFFICE_DISTRICT ASC";
    ...
    $('#" . $thisid . "').tablesorter({ sortlist: [0,0]});
		...
		";
    		...
    		";
                ...
                <table class='bordered tablesorter table-hover tablesaw tablesaw-stack' data-tablesaw-mode='stack' style='margin-top: 20px' id='$thisid'>
                    <thead>
                        <tr>
                            <th>ELECTION</th>
                            <th>COMMITTEE</th>
            ...
            ";
                ...
                function getfeccand18($committee)
                {
                    ...
                    $tmp = Array();
                    $retval = Array();
                    $sql = "SELECT * FROM nufec_ccl WHERE CMTE_ID = '$committee'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                    ...
                    $sql = "SELECT * FROM nufec_cn WHERE CAND_ID = '$x'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
...
";
                ...
                function gettoptwo($fourcode)
                {
                    ...
                    $sql = "SELECT * FROM calaccess_raw_cand_comm WHERE FOURCODE = '$fourcode'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $tmp['NAMF'] = $row['NAMF'];
