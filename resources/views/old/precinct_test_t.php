...
";
        ...
        */
	    ...
	    if(array_key_exists($precinct, $coordinates)) {
		//DO NOTHING
	    } else {
		continue;
	    }
		...
		if(array_key_exists($sd, $candidate_array)) {
	    ...
	    if (array_key_exists('.GOV', $candidate_array)) { $gov_cands = $candidate_array['.GOV']; }
	    if (array_key_exists('.LTG', $candidate_array)) { $ltg_cands = $candidate_array['.LTG']; }
	    if (array_key_exists('.ATG', $candidate_array)) { $atg_cands = $candidate_array['.ATG']; }
	    if (array_key_exists('.SOS', $candidate_array)) { $sos_cands = $candidate_array['.SOS']; }
            if (array_key_exists('.TRS', $candidate_array)) { $trs_cands = $candidate_array['.TRS']; }
            if (array_key_exists('.CON', $candidate_array)) { $con_cands = $candidate_array['.CON']; }
            if (array_key_exists('.INS', $candidate_array)) { $ins_cands = $candidate_array['.INS']; }
            if (array_key_exists('.SPI', $candidate_array)) { $spi_cands = $candidate_array['.SPI']; }
            if (array_key_exists('.USS', $candidate_array)) { $sen_cands = $candidate_array['.USS']; }
	    if (array_key_exists('.PRS', $candidate_array)) { $prs_cands = $candidate_array['.PRS']; }
		    ...
		    if(array_key_exists($key1, $votes)) {
			//DO NOTHING
		    } else {
			$votes[$key1] = [];
		    }

		    if(array_key_exists('TOTAL', $votes[$key1])) {
            ...
            foreach ($prs_cands as $value) {
                ...
                if (mb_substr($election, 0, 1) == "p") {
                    ...
                    switch ($party) {
                        case "REP":
                            $prs_cands[$distkey]['ID'] = 0;
                            break;
                        case "DEM":
                            $prs_cands[$distkey]['ID'] = 1;
                            break;
                        case "GRN":
                            $prs_cands[$distkey]['ID'] = 2;
                            break;
                        case "LIB":
                            $prs_cands[$distkey]['ID'] = 6;
                            break;
                        case "PAF":
                            $prs_cands[$distkey]['ID'] = 3;
                            break;
                        case "AIP":
                            $prs_cands[$distkey]['ID'] = 5;
                            break;
                        default:
                            $prs_cands[$distkey]['ID'] = 7;
                    }
	    ...
	    if(array_key_exists('TOTREG', $coordinates[$precinct]) && $coordinates[$precinct]['TOTREG'] > 0) {
		...
		$turnoutoffset = 0;
            ...
            if(array_key_exists('COORDINATES', $coordinates[$precinct]) && array_key_exists('POLYS', $coordinates[$precinct])) {
		//DO NOTHING
		//echo("<br>PROCESSING $precinct");
	    } else {
                //continue;
            }
            ...
            if (array_key_exists('POLYS', $coordinates[$precinct])) {
	  ...
	  function showArrays(event) {
        ...
        function get_overlaps($fourcode)
        {
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    if (array_key_exists('addist', $row)) {
        ...
        */
        ...
        function processboundaries($lat, $lng)
        {
            ...
            if (array_key_exists('lat_max', $boundaries)) { } else { $boundaries['lat_max'] = $lat; }
	    if (array_key_exists('lat_min', $boundaries)) { } else { $boundaries['lat_min'] = $lat; }
            if (array_key_exists('lng_max', $boundaries)) { } else { $boundaries['lng_max'] = $lng; }
	    if (array_key_exists('lng_min', $boundaries)) { } else { $boundaries['lng_min'] = $lng; }
	    ...
	    $distkey = '';
			...
			//echo("<br>POPULATING GET_CANDIDATES  $distkey - $name");
        ...
        function get_state_candidates()
        {
            ...
            //$distno = $x['distno'];
            //$sql = "SELECT distkey, party, name, is_incumbent, disttype, CAST(distnum AS UNSIGNED) AS distnum FROM candidates where disttype='$disttype' && distnum='$distno' && election = '$election'";
            $sql = "SELECT * FROM ctb2016_candidates WHERE (disttype IS NULL || disttype = '') && election = '$election'";
	    ...
	    //echo("<br>$sql<br>");
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $party = $row['party'];
                    $name = $row['name'];
		    //echo("<br>POPULATING GET_STATE_CANDIDATES  $distkey - $name");
                    $is_incumbent = $row['is_incumbent'];
        ...
        */
        ...
        function get_precincts($fourcode)
        {
            ...
            $sql = "SELECT srprec, county FROM ctb2016_$election WHERE $disttype = '$distno' AND TOTREG > 0";
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBH018pbcVij9pf5O0p-fkMI457nBV-keQ&sensor=falsesensor=false&callback=initMap"></script>
