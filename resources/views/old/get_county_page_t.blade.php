...
@section('title', 'County Page | California Target Book')
...
@section('content')
    ...
    $location_div = "<div style='float: left;' width='400px'><img width='400px' src='" . $images['LOCATION'] . "' /><p align='center' style='max-width: 400px;'>CITIES<br>$cities</p></div>";
    $supe_district_div = "<div align='justify' style='float: right; max-width: 600px'><a href='" . $images['DISTRICTS'] . "' target='_blank'><img width='600px' src='" . $images['DISTRICTS'] . "' /></a><p>$profile</p></div>";
    ...
    if ($county == "SAN FRANCISCO") {
        while ($i < 12) {
            ...
            $dist_iframes[$i] = "<iframe src='get_county_sub2.php?id=" . $county . "&sub=Supervisorial%20District%20" . $i . "' width='1024px' frameborder='0' scrolling='no' onload='resizeIframe(this)'></iframe>";

            $supe_tables[$i] = get_supe_table($i);
            $tmp = get_supe_incumbent($i);
        ...
        while ($i < 6) {
            $districts .= "<li><a href='#D$i'>D$i</a></li>";
            //$dist_iframes[$i] = "<iframe src='get_county_sub2.php?id=" . $county . "&sub=Supervisorial%20District%20" . $i . "' width='1024px' frameborder='0' scrolling='no' onload='resizeIframe(this)'></iframe>";
            $dist_iframes[$i] = "<iframe src='get_county_sub2.php?id=" . $county . "&sub=Supervisorial%20District%20" . $i . "' width='1040px' height='1280px'></iframe>";
            $supe_tables[$i] = get_supe_table($i);
...
<ul>
  ...
  <li><a href='#LocDetail'>Local Election Detail</a></li>
  <li><a href='#Maps'>District Maps</a></li>
    ...
    */
    ...
    $county_stats = "<iframe src='get_county_sub2.php?id=" . $county . "&sub=County%20Totals' width='1024px' frameborder='0' scrolling='no' onload='resizeIframe(this)'></iframe>";
...
";
    ...
    $i = 1;
    foreach ($dist_iframes as $f) {
        echo("<div id='D$i'>");
        echo($f);
        echo("<div style='float: left'>" . $supe_tables[$i] . "</div>");
        echo($supe_incumbent[$i]);
        echo("</div>");
        $i++;
    }
    ...
    $precinct_frame = "<iframe src='map_nav.php?id=CO" . $num . "' width='900px' height='950px'></iframe>";

    $local_detail_frame = "<iframe src='get_county_all.php?id=$county' width='100%x' height='1280'></iframe>";


    echo("<div id='Precincts'>$precinct_frame</div>");
    echo("<div id='LocDetail' width='100%' height='1280'>$local_detail_frame</div>");
    ...
    */
    ...
    echo("<div id='Maps' width='100%' height='1200'>$test_frame" . $map_frame . "</div>");
    ...
    function get_cities()
    {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $city = $row['name'];
                $link = "<a href='get_county_sub2.php?id=$county&sub=$city' target='_blank'>$city</a>" . ", ";
                $retval .= $link;
            }
        }
    ...
    function get_supe_incumbent($district)
    {
        global $site_conn;
        global $county;
        $old_dist = $district;
        ...
        $html = "<div style='float: right; max-width: 600px; display: inline-block;' align='center'>
    ...
    function convert_supe($county)
    {
        ...
        return $conversion_array[$this_county];
    ...
    function retrieve_bio($id)
    {
        global $fec_conn;
        $conn = Util::get_ctb_conn();
        $sql = "SELECT bio FROM ctb_bios WHERE bioguide = '$id'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $retval = $row['bio'];
    ...
    function drawreg($fourcode)
    {
        ...
        if (mb_substr($fourcode, 0, 2) == "CA") {
            ...
            $dist = mb_substr($fourcode, 2, 2);
            $adjusted = "CD" . $dist;
            $sql = "SELECT * FROM ctb_sos_oct16 WHERE DIST = '$adjusted'";
        } else {
            global $fl_conn;
            $conn = Util::get_ctb_conn();
