...
$incumbent = get_incumbent($id);


echo("<div class='jumbotron text-center sacto'>
  <h1>$id</h1>
  $prev_div
  $next_div
  $incumbent
...
function get_links($fourcode)
{
    ...
    $prev_url = get_page_from_offset($prev_offset) . $prev;
    $next_url = get_page_from_offset($next_offset) . $next;
...
{
    if ($number < 177) {
        $retval = 'get_ctb_page_b.php?id=';
    }

    if ($number > 176 && $number < 235) {
        $retval = 'get_county_page_b.php?id=';
    }

    if ($number > 234 && $number < 670) {
        $retval = 'get_fed_page_b.php?id=';
    }
...
function get_incumbent($id)
{
    ...
    if ($use_cal) {
        $sql = "SELECT * FROM e18_incumbent WHERE DIST = '$id'";
        ...
        $sql = "SELECT * FROM e18_fed WHERE DIST = '$id'";
    ...
    $retval = "<h3 align='center'>$name ($party)" . $term_append . "</h3>";
...
function is_open_fed($fourcode)
{
    global $fec_conn;
    $conn = $fec_conn;
    $sql = "SELECT * FROM e18_open WHERE fourcode = '$fourcode'";
    //echo($sql);
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
    ...
    if ($reason) {
        $retval = "<p style='font-size: 1.4em;'><span class='boldme'>OPEN SEAT IN 2018<br>" . $incumbent . " (" . $party . ")" . " is " . $reason . "</span></p>";
    } else {
        $retval = '';
    }
