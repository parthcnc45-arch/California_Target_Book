...
@section('title', '2022 Proposition Finances | California Target Book')
  ...
  foreach($arr as $id => $x) {
    ...
    foreach($cmtes[$id] as $position => $arr2) {
      ...
      foreach($arr2 as $cmte_id => $y) {
        ...
        if(isset($s['LAST_FILING'])) {
          $filing_url = "<a href='https://cal-access.sos.ca.gov/PDFGen/pdfgen.prg?filingid=" . $s['LAST_FILING'] . "' target='_blank'>" . $s['LAST_FILING'] . "</a>";
        } else {
          $filing_url = "NO RPT YET";
        }
                    ...
                    <table align='center' class='table table-striped table-hover table-fit'>
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td align='right'>\$" . number_format($s['LAST_RCPT']) . "</td>
                          <td align='right'>\$" . number_format($s['LATE_CONTRIBUTIONS']) . "</td>
                          <td align='right' class='boldme'>\$" . number_format($s['LIFETIME_RCPT']) . "</td>
                          <td align='right'>\$" . number_format($s['LIFETIME_EXPN']) . "</td>
                          <td align='right'>\$" . number_format($s['DEBTS']) . "</td>
                          <td align='right'>\$" . number_format($s['LOANS']) . "</td>
                          <td align='right'>\$" . number_format($s['COH']) . "</td>
        ...
        $total_spend += $s['LIFETIME_RCPT'];
        $prop_spend[$prop_id][$position] +=$s['LIFETIME_RCPT'];
        $prop_spend_tot[$prop_id] += $s['LIFETIME_RCPT'];
...
foreach($prop_index_all as $prop_id => $x)                     {
  ...
  $sup_tot += $prop_spend[$prop_id]['SUPPORT'];
  $opp_tot += $prop_spend[$prop_id]['OPPOSE'];

  $prop_summary_table .= "<tr>
			    <td>" . $x['ballot_no'] . "</td>
                            <td><a href='/book/propositions/$prop_id' target='_blank'>$prop_id</a></td>
                            <td>" . mb_substr($x['prop_nm'], 0, 128) . "</td>
                            <td align='right'>\$" . number_format($prop_spend[$prop_id]['SUPPORT']) . "</td>
                            <td align='right'>\$" . number_format($prop_spend[$prop_id]['OPPOSE']) . "</td>
                            <td align='right'>\$" . number_format($prop_spend[$prop_id]['OPPOSE'] + $prop_spend[$prop_id]['SUPPORT']) . "</td>
                          </tr>";
}
...
$page[101] = "<div class='prop_div'>
                ...
                <h2 align='center'>PROPOSITION SUMMARY</h2>
                <div class='table-responsive'>
                $prop_summary_table
                </div>
                <hr />
...
foreach($statuses as $status => $verbose) {
  uasort($div_index_amt[$status], "totalsort");
...
function get_props() {
  ...
  $sql = "SELECT prop_id, fppc_id, ballot_no, prop_dscr, prop_status, prop_nm, sigs_needed, sigs_deadline, session, sigs_submitted, sigs_valid
          ...
          WHERE session = 2021 && prop_status != 0 && prop_id != 1438975
