...
@section('title', "G12 Federal Campaign Finance Summaries for $state")
    ...
    <div class='container'>
        <div class='row'>
            <div class='col-lg-12'>
                ...
                $sql = "SELECT CAND_ID, CAND_NAME, CAND_PTY_AFFILIATION, CAND_ELECTION_YR, CAND_OFFICE_ST, CAND_OFFICE, CAND_OFFICE_DISTRICT, CAND_PCC, CAND_ICI FROM nufec_cn_12 WHERE CAND_ELECTION_YR = '2012' && CAND_OFFICE_ST = '$thisstate' ORDER BY CAND_OFFICE_ST, CAND_OFFICE, CAND_OFFICE_DISTRICT ASC";
...
//                echo($tablehead);
                            ...
                            if ($thisdist != $lastdist || $thisdist == '') {
                                ...
                                if (($thisdist == "0" || $thisdist == '') && !$runonce) {
                                    ...
                                    if ($row['CAND_OFFICE_DISTRICT'] == "00" || $row['CAND_OFFICE_DISTRICT'] == '') {
                                        ...
                                        $fourcode_url = "<a href='get_fed_page_t.php?id=$fourcode'>$thisstate At Large House Seat</a>";
                                    }
                                    $is_open = is_open();
                                    $is_targeted = is_targeted();
                                    $drawentry .= "<div class='newseg'><h1>$fourcode_url</h1>";
                                    ...
                                    $fourcode = $thisstate . checkaddzero($row['CAND_OFFICE_DISTRICT']);
                                    $fourcode_url = "<a href='get_fed_page_t.php?id=$fourcode'>$fourcode</a>";
                                    $drawentry .= "<div class='newseg'><h1>$fourcode_url</h1>";
            ...
            ";
                ...
                function getfeccand18($committee)
                {
                    ...
                    $tmp = Array();
                    $retval = Array();
                    $sql = "SELECT * FROM nufec_ccl_12 WHERE CMTE_ID = '$committee'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                    ...
                    $sql = "SELECT * FROM nufec_cn_12 WHERE CAND_ID = '$x'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                ...
                function getfecsum18($candidate)
                {
                    ...
                    $tmp = Array();
                    $retval = Array();
                    $sql = "SELECT * FROM nufec_weball_12 WHERE CAND_ID = '$candidate'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
...
";
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $date = $row['rec_dat'];
                            $payee = $row['pay'];
                            $filing_link = "<a href='fedparser.php?id=$filing_id' target='_blank'>$filing_id</a>";
