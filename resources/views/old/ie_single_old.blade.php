...
@section('content')
        ...
        foreach ($value as $cand) {
            ...
            foreach ($s496 as $filing) {
                if ($filing['TOTAL']) {
                    ...
                    $f_link = "<a href='/book/cmlocal2.php?id=$filer_id' target='_blank'>$tmp</a>";
	...
	";
        ...
        foreach ($endfile as $entity) {
            ...
            $filer = "<a href='cmlocal2.php?id=$filer_id' target='_blank'>$filer</a>";
        ...
        if ($totaliespending) {
            if ($totalprimary) {
                ...
                echo("<p><b style='color: #4B57B1;'>Independent Expenditure Activity in Primary:</b>&nbsp;<b style='color: FireBrick;'>TOTAL IE SPENDING \$" . number_format($totalprimary, 2) . "</b><p>");
            ...
            if ($totalgeneral) {
                echo("<div class='row'>");
                echo("<div class='col-lg-12'>");
                echo("<p><b style='color: #4B57B1;'>Independent Expenditure Activity in General (Updated " . date("m-d") . "):</b>&nbsp;<b style='color: FireBrick;'>TOTAL IE SPENDING \$" . number_format($totalgeneral, 2) . "</b><p>");
            ...
            if ($totaliespending) {
                echo("<h1 class='redme'>Total IE Spending: \$" . number_format($totaliespending, 2) . "</h1>");
            }
    ...
    if ($enddraw) {
        ...
        echo("<div class='col-lg-12'>");
        echo("<input type='button' value='Show Expenditure Detail Table' onclick='toggleTable();'>");
	...
	";
        ...
        echo("<div class='row'><div class='col-lg-12'><p align='center' style='font-family: \"Lato\"; font-size: 1.3em; font-weight: bold; font-variant: small-caps;'>No Independent Expenditures Recorded in $year</p></div></div>");
    }
    ...
    function fetchcandidates($fourcode)
    {
        ...
        $dist_type = mb_substr($fourcode, 0, 2);
        ...
        $sql = "SELECT * FROM calccess_raw_CVR_CAMPAIGN_DISCLOSURE_CD
			WHERE DIST_NO = $dist_no && FORM_TYPE = 'F496' $excep
			GROUP BY CAND_NAML, DIST_NO";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                foreach ($arr as $value) {
                    if ($value == "(I)" || $value == "Sr." || $value == "Jr.") {
                        //DO NOTHING
                    } else {
                        $last = $value;
                    }
                }
                $retval[$last]['NAME'] = strtoupper($last);
            }
        }
        ...
        foreach ($filings as $value) {
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($expdate > "$year-12-31" || $expdate < "$year-01-01") {
                        ...
                        $filer_lnk = "<a href='book/cmlocal2.php?id=" . $value['FILER_ID'] . "' target='_blank'>$filer_name</a>";
