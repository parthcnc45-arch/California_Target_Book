...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
  ...
  <link rel="stylesheet" type="text/css" href="tr/style/tabs.css" />
  <link rel="stylesheet" type="text/css" href="t/style/tabstyles.css" />
  ...
  <script src="t/js/modernizr.custom.js"></script>
...
foreach($cand_finances as $cand_id => $c) {
	...
	if($c['cmte_id']) {
		if(mb_substr($c['cmte_id'], 0, 1) == "C" ) {
			...
			$cand_span = "<a href='$base_fppc_cand" . $c['cand_id'] . "' target='_blank'>" . $c['cand_id'];
		}
	} else {
...
foreach($etypes as $etype) {
	if($totals['elections'][$etype]['summary']['TOTAL']) {
		...
		$js_functions .= "
						  draw" . $first_three . "Pie();
						  draw" . $first_three . "Date();
						  draw" . $first_three . "Supopp();
		";
		...
		foreach($filer_election_totals[$etype] as $filer_id => $amt) {
			...
			$pie_function = "draw" . $first_three . "Pie";
			$supopp_function = "draw" . $first_three . "Supopp";
...
$js = "google.load('visualization', '1.0', {'packages':['corechart'], 'callback': drawCharts});
...
foreach($etypes as $etype) {
	$first_three = mb_substr($etype, 0, 3);
	$date_function = "draw" . $first_three . "Date";
...
foreach($etypes as $etype) {
	...
	if($totals['elections'][$etype]['summary']['TOTAL']) {
		...
		$js = "function draw" . $first_three . "Pie() {
...
$js = "        
			$('#page_candidates').removeClass('active');
			$('#page_candidate_committees').removeClass('active');
			$('#page_ie_detail').removeClass('active');
			$('#page_cand_graphs').removeClass('active');
			$('#page_combined_graphs').removeClass('active');";
array_push($endjava, $js);
...
$candidates_page = "<div id='candidates_tab' class='tabs tabs-style-bar' style='width: 100%; background-color: white;'>";
$candidates_page .= $page['candidates'];
$candidates_page .= "</div>";

$ie_detail_page = "<div id='ie_details_tab' class='tabs tabs-style-bar' style='width: 100%' background-color: white;'>";
$ie_detail_page .= "<ul>                 
                      <li><a href='#ie_cmte_totals_page'  class='fa fa-lg fa-file-invoice-dollar'><span>CMTE TOTALS</span></a></li>
                      <li><a href='#ie_detail_table_page' class='fa fa-lg fa-file-invoice-dollar'><span>IE DETAIL</span></a></li>
                    </ul>";
$ie_detail_page .= "<div id='ie_cmte_totals_page' align='center'><h1>IE SUMMARY BY COMMITTEE</h1>" . $page['ie_cmte_totals'] . $page['ie_election_totals'] . "</div>";
$ie_detail_page .= "<div id='ie_detail_table_page' align='center'><h1>INDEPENDENT EXPENDITURES FOR $master_fourcode ($cycle Cycle)<br>\$" . number_format($totals['grand_total']) . "</h1>$e_breakout" . $page['ie_detail'] . "</div>";
$ie_detail_page .= "</div>";

$candidate_committees_page = "<div id='cand_committees_tab' class='tabs tabs-style-bar' style='width: 100%; background-color: white;'>";
...
$end_draw = "<div class='container'>
				<div class='tabs tabs-style-bar' width='100%' style='background-coolor: white;'>
					<nav class='tab-bar' width='100%'>
						<ul class='nav nav-tabs' role='tablist'>
							<li><a href='#page_candidates' data-toggle='tab' class='fa fa-lg fa-info-circle'><span>CANDIDATES</span></a></li>
							<li><a href='#page_candidate_committees' data-toggle='tab' class='fa fa-lg fa-money-bill-wave'><span>CAND COMMITTEES</a></li>
							<li><a href='#page_ie_detail' data-toggle='tab' class='fa fa-lg fa-comment-dollar'><span>IE DETAIL</span></a></li>
							<li><a href='#page_ie_graphs' data-toggle='tab' class='fa fa-lg fa-chart-pie'><span>IE GRAPHS</span></a></li>
							<li><a href='#page_cand_graphs' data-toggle='tab' class='fa fa-lg fa-chart-bar'><span>CAND GRAPHS</span></a></li>
							<li><a href='#page_combined_graphs' data-toggle='tab' class='fa fa-lg fa-chart-line'><span>COMBINED GRAPHS</span></a></li>

						</ul>
					</nav>
					<div class='tab-content'>
						<div class='tab-pane active' id='page_candidates' style='background-color: white;'>
							$candidates_page
						</div>

						<div class='tab-pane active' id='page_candidate_committees' style='background-color: white;'>
							$candidate_committees_page
						</div>

						<div class='tab-pane active' id='page_ie_detail' style='background-color: white;'>
							$ie_detail_page
						</div>

						<div class='tab-pane active' id='page_ie_graphs' style='background-color: white;'>
							$chart_divs
						</div>

						<div class='tab-pane active' id='page_cand_graphs' style='background-color: white;'>
							$candidate_graphs_page
						</div>

						<div class='tab-pane active' id='page_combined_graphs' style='background-color: white;'>
...
<script>
  ...
  $( function() {
    $( "#page_candidates" ).tabs();
  } );  

  $( function() {
    $( "#page_candidate_committees" ).tabs();
  } );  
...
$( function() {
    $( "#page_cand_graphs" ).tabs();
  } );

  $( function() {
    $( "#page_combined_graphs" ).tabs();
  } ); 
