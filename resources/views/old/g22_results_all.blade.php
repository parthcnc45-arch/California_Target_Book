...
@section('title', 'November 8th, 2022 General Results - All | California Target Book')
...
<div class='container-fluid pt-lg' height='100%'>
    <h2>
        November 8th, 2022 General Results - All
    </h2>
...
foreach($last_votes as $fourcode => $cands) {
	...
	foreach($cands as $cand_id => $x) {
		...
		if(empty($end_arr_other[$fourcode]['party_votes'][$x['party']])) {
			$end_arr_other[$fourcode]['party_votes'][$x['party']] = 0;
		}
...
foreach($end_arr as $fourcode => $ranks) {
	...
	$r_vot = ($end_arr_other[$fourcode]['party_votes']['Rep'] ?? 0);
	$d_vot = ($end_arr_other[$fourcode]['party_votes']['Dem'] ?? 0);
	$d_pct = number_format((($d_vot / $total) * 100), 2);
	$r_pct = number_format((($r_vot / $total) * 100), 2);
	if(empty($enddraw[$type])) {
		$enddraw[$type] = '';
	}
...
$types = Array("STW", "PROP", "AD", "SD", "CD", "BOE");
$is_active = TRUE;
...
function populate_last() {
	...
	$sql = "SELECT * FROM (
					SELECT fourcode, update_number FROM ctb_g22_results_state
	...
	$sql = "SELECT * FROM ctb_g22_results_state 
			WHERE ( $query )";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
