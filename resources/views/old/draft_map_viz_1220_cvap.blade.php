...
@section('title', 'Final Maps CVAP | California Target Book')
...
<div class='container-fluid pt-lg' height='100%'>
    <h2>
        Final Maps Citizen Voting Age Population (CVAP) Data
    </h2>
...
function generate_table($arr, $type, $target, $incumbents) {
  global $endjava_sections, $old_fourcodes, $map_name_index;
  $old_fourcodes = get_fourcode_index();
  ...
  $js = "$(document).ready(function() {
    ...
    foreach($arr as $place => $x)  {
      ...
      //echo("<br>$fourcode YIELDS $old_fourcode");
...
function populate_draft() {
	//global $master_conn;
	//$conn = $master_conn;
	global $old_fourcodes;
	$conn = Util::get_ctb_conn();

	$sql = "SELECT jur_name, district FROM ctb_ca_city_shp WHERE jur_name LIKE 'VIZ8_%'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$jur_name = $row['jur_name'];
			$jur_name = str_replace("VIZ8_", "", $jur_name);
...
function get_cvap($dists) {
	...
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$fourcode = $row['fourcode'];
			$old_fourcode = $old_fourcodes['old'][$fourcode];
	...
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			$fourcode = $row['fourcode'];
			$map_name_index[$fourcode] = $map_nm;
			//$fourcode = $type . checkaddzero($row['district']);
			//$old_fourcode = $old_fourcodes['old'][$fourcode];
	...
	foreach($dists as $fourcode => $ignore) {
		...
		$tmp[2021]['mapname'] = $map_name_index[$fourcode];
...
function get_colors($section) {
  switch($section) {
    case "AD":
      //DARK BLUE - ASSEMBLY
      $head = "#00334d";
      $columns = Array("#ffffff", "#ffffff", "#ffffff","#e6f7ff", "#e6f7ff", "#e6f7ff", "#e6f7ff", "#ecf9ec", "#ecf9ec", "#ecf9ec", "#ecf9ec", "#ecf9ec", "#fffee3", "#fffee3", "#fffee3", "#fffee3", "#fff0e6", "#fff0e6", "#fff0e6", "#fff0e6");
      break;
    case "SD":
