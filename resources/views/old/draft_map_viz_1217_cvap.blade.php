...
<div class='container-fluid pt-lg' height='100%'>
    <h2>
        December 17th SD Visualization Citizen Voting Age Population (CVAP) Data
    </h2>
...
function get_cvap($dists) {
	...
	$jur_name = "VIZ6_";
	$sql = "SELECT district, name, jur_name FROM ctb_ca_city_shp WHERE jur_name = 'VIZ6_SD'";
	//echo("\n$sql");
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
	...
	$sql = "SELECT name, population, cvap_19, hisp, ind, black, asian, white, target, deviation FROM ctb_redist_cvap_1217";
	//echo("\n$sql");	
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
