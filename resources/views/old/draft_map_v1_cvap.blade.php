...
@section('title', 'Draft Map v1 CVAP | California Target Book')
...
<div class='container-fluid pt-lg' height='100%'>
    <h2>
        Draft Map v1 Citizen Voting Age Population (CVAP) Data
    </h2>
...
function populate_draft() {
	...
	$sql = "SELECT jur_name, district FROM ctb_ca_city_shp WHERE jur_name LIKE 'VIZ5_%'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$jur_name = $row['jur_name'];
			$jur_name = str_replace("VIZ5_", "", $jur_name);
