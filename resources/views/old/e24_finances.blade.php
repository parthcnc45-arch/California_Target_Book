...
@section('title', '2024 FPPC / FEC Finance Summaries | California Target Book')
...
<div class='container-fluid pt-lg' height='100%'>
    <h2>
        2024 FPPC / FEC Finance Summaries
    </h2>
...
foreach($sections as $section => $verbose) {
   ...
   if($section == "CONSTITUTIONAL") {
	  ...
	  $t = get_fed(".SN1", $section);

          $enddraw .= "<div class='panel justifyme'> <!--BEGIN US SENATE PANEL DIV -->
                        <p style = 'text-align: center !important;'>US SENATE - \$" . number_format($dist_totals['.SN1']) . " Raised</p>";         
          $enddraw .= $t;
...
function get_fec_summary($candidate) {
  ...
  $tmp = Array();
  $retval = Array();
  $sql = "SELECT * FROM nufec_weball_24 WHERE CAND_ID = '$candidate'";
  //echo("<Br>$sql<br>");
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
...
function populate_districts() {
  ...
  $conn = Util::get_ctb_conn();
  $retval = Array();
  $sql = "SELECT * FROM ctb_cand_filed_v2 WHERE year = '2024' GROUP BY fourcode ORDER BY fourcode";
  $result = $conn->query($sql);
  if($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
...
function getstatecommittees($dist) {
  ...
  $retval = Array();
  $tmp =  Array();
  $sql = "SELECT * FROM ctb_cand_filed_v2 WHERE (year = '2023' || year = '2024') && (type = 'P' || type = 'S') && fourcode = '$dist' && toptwo != 3 && hide != 1";
  $result = $conn->query($sql);
  if($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      $tmp['namf'] = $row['namf'];
