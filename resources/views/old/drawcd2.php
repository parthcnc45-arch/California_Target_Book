...
function get_stats($precinct, $county)
{
    ...
    global $election;
    $conn = $ctb2016_conn;
    $sql = "SELECT * FROM g16_temp WHERE pct16 = '$precinct'";
...
function get_g16_precinct_polygons($id)
{
    ...
    $sql = "SELECT ST_AsText(SHAPE) AS SHAPE FROM house WHERE geoid = '$id'";
...
function convertfourcode($fourcode)
{
    ...
    $sql = "SELECT CODE FROM codes WHERE STATE = '$state'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $code = $row['CODE'];
...
function getallfourcodes()
{
    ...
    $conn = $ctb2016_conn;
    $retval = Array();
    $sql = "SELECT DIST FROM e18_fed GROUP BY DIST";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $dist = $row['DIST'];
