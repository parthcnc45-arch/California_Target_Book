...
@section('title', 'FEC Social Media | California Target Book')
...
@section('content')
    ...
    function retrieve_social_media($fourcode)
    {
        global $ctb2016_conn;
        $conn = Util::get_ctb_conn();
        $sql = "SELECT TWITTER, FACEBOOK, INSTAGRAM, YOUTUBE FROM ctb2016_e18_fed WHERE DIST = '$fourcode'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $retval['TWITTER'] = $row['TWITTER'];
                $retval['FACEBOOK'] = $row['FACEBOOK'];
                $retval['INSTAGRAM'] = $row['INSTAGRAM'];
                $retval['YOUTUBE'] = $row['YOUTUBE'];
            }
        }
