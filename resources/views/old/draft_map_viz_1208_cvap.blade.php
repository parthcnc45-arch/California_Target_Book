...
function generate_table($arr, $type, $target, $incumbents) {
  global $endjava_sections, $old_fourcodes;
  $old_fourcodes = get_fourcode_index();
...
function populate_draft() {
	...
	$sql = "SELECT jur_name, district FROM ctb_ca_city_shp WHERE jur_name LIKE 'VIZ7_%'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$jur_name = $row['jur_name'];
			$jur_name = str_replace("VIZ7_", "", $jur_name);
