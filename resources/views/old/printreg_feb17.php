...
echo("<br>FEB '17:<br>Dem Advantage: $demadv<br>Rep Advantage: $gopadv<br>");
...
function getreg($fourcode) {
	...
	$sql = "SELECT * FROM  sos_feb17 WHERE DIST = '$fourcode'";
...
function getofficeholder($fourcode){
	global $ctb2016_conn;
	$conn = $ctb2016_conn;
	$sql = "SELECT LEGISLATOR, PARTY FROM e18_incumbent WHERE DIST = '$fourcode'";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
