...
@section('title', 'Recent FEC Form 1 Filings | California Target Book')
...
foreach($arr as $filing_id => $x) {
	...
	$bank = generate_bank($x);
	$cmte_info = generate_cmte_info($x);
	$cmte_type_info = generate_cmte_type_info($x);
	$tbody .= "<tr>
					<td>" . $x['filing_id'] . "</td>
					<td>" . $x['filer_committee_id_number'] . "</td>
					<td>" . $x['committee_name'] . "</td>
					<td>" . $cmte_type_info . "</td>
					<td>" . $cmte_info . "</td>
					<td>" . $sig . "</td>
					<td>" . $trs . "</td>
					<td>" . $bank . "</td>
	...
	$table = "<table class='table table-striped' id='$thisid'>
				<thead>
					<tr>
						<th>FILING</th>
						<th>CMTE ID</th>
...
function populate_last() {
	...
	$sql = "SELECT * FROM ctb_fec_f1_db WHERE form_type = 'F1N' ORDER BY filing_id DESC LIMIT 100";
