...
<html lang="en">
    ...
    <body id="page-top" data-spy="scroll" data-target=".navbar-fixed-top">
        ...
        if (mb_substr($d1f, 0, 2) == "CO") {
            $d1lc = " (" . get_long_county(mb_substr($d1f, 2, 2)) . ")";
        }

        if (mb_substr($d2f, 0, 2) == "CO") {
            $d2lc = " (" . get_long_county(mb_substr($d2f, 2, 2)) . ")";
        }

        $headline = "<div style='max-width: 800px; display: block; border: 2px solid black; align: left;'><p align='center'>YELLOW: " . $d1f . $d1lc . " " . $d1s . " - " . $d1y . " Boundaries<br>BLUE: " . $d2f . $d2lc . " " . $d2s . " - " . $d2y . " Boundaries<br>GREEN: OVERLAP</p></div><div width='100%' style='clear: both;'></div>";

        echo($headline);


        $g16precincts_conn = new mysqli($servername, $username, $password, $dbname);
        ...
        */
        ...
        //echo("<br>USING DISTRICTs $d1f, $d2f -- PRECINCT DUMP:<br>");
        //var_dump($precincts);
        ...
        */
        ...
        foreach ($precincts as $precinct) {
            ...
            $coordinates[$srprec]['USE_SUPE'] = $this_supe;
...
";
        ...
        */
        ...
        foreach ($precincts as $value) {
            ...
            $srprec = $value['id'];
            $precinct = $srprec;
            $use_supe = $coordinates[$precinct]['USE_SUPE'];
	  ...
	  function showArrays(event) {
        ...
        if(mb_substr($district, 0, 3) == "ALL") {
            ...
            foreach($omni as $k => $v) {
                ...
                foreach($v as $e => $arr) {
                    ...
                    if($arr['REP'] > $arr['DEM'] && $arr['PARTY'] == "D") {
                        $d_in_r[$e]++;
                    }

                    if($arr['DEM'] > $arr['REP'] && $arr['PARTY'] == "R") {
                        $r_in_d[$e]++;
                    }

                    $officeholders[$e] .= "<br>" . $this_fourcode . " - " . $arr['NAME'] . " (" . $arr['PARTY'] . ")";
            ...
            foreach($hdrs as $e) {
                echo("<br>$e -- R: " . $leg_comp[$e]['R'] . ", D: " . $leg_comp[$e]['D'] . " | " . $rep_adv[$e] . " R Districts, " . $dem_adv[$e] . " D Districts, " . $d_in_r[$e] . " Dem Officeholders in GOP Districts, " . $r_in_d[$e] . " GOP Officeholders in Dem Districts" );
                //echo($officeholders[$e]);
            }
        ...
        */
        ...
        function get_district_color($fourcode)
        {
            ...
            if ($colors[$fourcode]['D'] && $colors[$fourcode]['R']) {
                $retval = "#FF00FF";
            } elseif ($colors[$fourcode]['D'] && !$colors[$fourcode]['R']) {
                $retval = "#0000FF";
            } elseif ($colors[$fourcode]['R'] && !$colors[$fourcode]['D']) {
                $retval = "#FF0000";
            }
        ...
        function lookup_results($geoid)
        {
            global $site_conn;
            $conn = $site_conn;
            $sql = "SELECT * FROM mtal WHERE geoid = '$geoid' ORDER BY updated DESC LIMIT 1";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $d_pct = number_format((($row['quist'] / $row['total']) * 100), 2);
                    $r_pct = number_format((($row['gianforte'] / $row['total']) * 100), 2);
                    $l_pct = number_format((($row['wicks']) * 100), 2);
                }
            }

            if ($d_pct > $r_pct) {
                $strength = $d_pct - $r_pct;
                if ($strength > 0) {
                    $color = get_blue(0);
                }
        ...
        */
        ...
        function get_stats($precinct)
        {
            ...
            $conn = $conn;
            $sql = "SELECT fourcode FROM ca_legislative WHERE OGR_FID = '$precinct'";
            ...
            global $ctb2016_conn;
            $conn = $ctb2016_conn;
            $sql = "SELECT * FROM caleg_past_officeholders WHERE dist = '" . $retval['fourcode'] . "' ORDER BY YR_END DESC";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    if ($row['PARTY'] == "R") {
                    ...
                    if ($boldclass && $row['PARTY'] == "R") {
                        $colors[$fourcode]['R']++;
                    } elseif ($boldclass && $row['PARTY'] == "D") {
                        $colors[$fourcode]['D']++;
                    }
        ...
        */
        ...
        function get_reg_table($fourcode)
        {
            ...
            global $omni;
            $conn = $ctb2016_conn;
            $sql = "SELECT * FROM dist_reg_hist WHERE FOURCODE = '$fourcode'";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $y = $row;
            ...
            if ($use_county) {
                ...
                $sql = "SELECT * FROM ca_county_census_stats WHERE COUNTY = '$county_nm'";
                //echo($sql);
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
        ...
        function get_precincts($fourcode)
        {
            ...
            $sql = "SELECT srprec, county FROM $election WHERE $disttype = '$distno'";
        ...
        function get_g16_precincts($fourcode)
        {
            ...
            $sql = "SELECT * FROM g16_temp WHERE pct16 LIKE '$county%'";
            //echo("<br>RETRIEVING PRECINCTS USING<br>$sql<br>");
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
        ...
        function get_null_precincts()
        {
            ...
            $sql = "SELECT * FROM g16_temp WHERE pct16 = '$fourcode'";
            echo("<br>RETRIEVING PRECINCTS USING<br>$sql<br>");
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
        ...
        */
        ...
        function get_county_ids($district, $sub)
        {
            ...
            if ($district == "ALL_CO" && $sub == "ALL") {
                $sql = "SELECT OGR_FID FROM supe_dist WHERE district != ''";
            } elseif (mb_substr($district, 0, 2) == "CO" && $sub == "ALL") {
                $n = mb_substr($district, 2, 2);
                $county_nm = get_long_county($n);
                $sql = "SELECT OGR_FID FROM supe_dist WHERE district != '' && county_nm = '$county_nm'";
            } else {
                $n = mb_substr($district, 2, 2);
                $county_nm = get_long_county($n);
                $sql = "SELECT OGR_FID FROM supe_dist WHERE district = '$sub' && county_nm = '$county_nm'";
            }
        ...
        function get_dual_ids()
        {
            ...
            if ($d1s) {
                //USE SUPERVISORIAL DISTRICT FOR D1
                $id1['id'] = get_county_ids($d1f, $d1s);
                $id1['use_supe'] = TRUE;
            } else {
                $id1['id'] = get_leg_ids($d1f, $d1y);
                $id1['use_supe'] = FALSE;
            }

            if ($d2s) {
                $id2['id'] = get_county_ids($d2f, $d2s);
                $id2['use_supe'] = TRUE;
            } else {
                $id2['id'] = get_leg_ids($d2f, $d2y);
                $id2['use_supe'] = FALSE;
            }

            $retval = Array($id1, $id2);
        ...
        function get_leg_ids($district, $year)
        {
            ...
            if ($district == "ALL_AD") {
                $sql = "SELECT OGR_FID FROM ca_legislative WHERE year = '$year' && fourcode LIKE 'AD%'";
            } elseif ($district == "ALL_SD") {
                $sql = "SELECT OGR_FID FROM ca_legislative WHERE year = '$year' && fourcode LIKE 'SD%'";
            } elseif ($district == "ALL_CD") {
                $sql = "SELECT OGR_FID FROM ca_legislative WHERE year = '$year' && fourcode LIKE 'CD%'";
            } elseif ($district == "ALL_CO") {
                $sql = "SELECT OGR_FID FROM ca_legislative WHERE fourcode LIKE 'CO%'";
            } elseif (mb_substr($district, 0, 2) == "CO") {
                $sql = "SELECT OGR_FID FROM ca_legislative WHERE fourcode = '$district'";
            } else {
                $sql = "SELECT OGR_FID FROM ca_legislative WHERE year = '$year' && fourcode = '$district'";
            }

            //echo("<br>USING SQL:<br>$sql<br>");
        ...
        function get_g16_precinct_polygons($id)
        {
            ...
            $sql = "SELECT ST_AsText(SHAPE) AS SHAPE FROM ca_legislative WHERE OGR_FID = '$id'";
        ...
        function get_supe_polygons($id)
        {
            ...
            $sql = "SELECT ST_AsText(SHAPE) AS SHAPE FROM supe_dist WHERE OGR_FID = '$id'";
        ...
        function get_precinct_polygons($srprec, $county)
        {
            ...
            if (!$has_alpha) {
                $srprec = $numcheck;
                $sql = "SELECT CAST(SRPREC AS UNSIGNED) AS SRPREC,  ST_AsText(SHAPE) AS SHAPE FROM precincts WHERE SRPREC = $srprec && COUNTY = '$county' && ELECTION = '$election'";
            } else {
                $sql = "SELECT ST_AsText(SHAPE) AS SHAPE FROM precincts WHERE SRPREC = '$srprec' && COUNTY = '$county' && ELECTION = '$election'";
            }
