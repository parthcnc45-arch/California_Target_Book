    ...
    $r_primary_votes = 0;
        ...
        if ($year > 2011) {
            ...
            foreach ($primary as $x) {
                ...
                $vote_pct = makepct2($x['votes'], $p_votes);
            ...
            $r_primary_body = '';
                foreach ($r_primary as $x) {
                    ...
                    $cand_img = get_candidate_image_sm($x['cand_id']);
                    $vote_pct = makepct2($x['votes'], $r_primary_votes);
                    $bg_class = 'redbg';
                    $r_primary_body .= "
                                    <tr class='$row_class'>
                                        <td class='img_cell'>$cand_img</td>
            ...
            $d_primary_body = '';
                ...
                foreach ($d_primary as $x) {
                    $vote_pct = makepct2($x['votes'], $d_primary_votes);
                    if ($i == 0) {
                        $row_class = 'winner';
                        $check_mark = "<span class='greenme boldme'>&#x2714;</span>";
                    } else {
        ...
        foreach ($general as $x) {
            ...
            $vote_pct = makepct2($x['votes'], $g_votes);
            $cand_img = get_candidate_image_sm($x['cand_id']);
        ...
        foreach ($candidates as $cand_id) {
            ...
            $cand_div= '<div class="mt-3 col-md-4 d-flex align-items-stretch">
                <div class="analysis-condidates bg-white pb-4 ctb-border-radius">
                    <div class="ctb-analysis-profile-img">
                        <img src="'.url("assets/imgs/img-16.png").'" alt="Banner Img" class="img-fluid flag-image" />
                    </div>
