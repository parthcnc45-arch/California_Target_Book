...
@section('title', '2020 Watchlist | California Target Book')
...
<div class="container-fluid pt-lg">
    ...
    <div class="mt-xl">
        @php( include Util::$view_root.'p20_filing_status.php' )
    </div>
</div>
...
function populate_watch() {
    ...
    $retval['active']='';
    $retval['inactive']='';
	$sql = "SELECT * FROM ctb_e20_county_watch WHERE status = '1' ORDER BY county_nm ASC";
	...
	$sql = "SELECT * FROM ctb_e20_county_watch WHERE status IS NULL ORDER BY county_nm ASC";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
